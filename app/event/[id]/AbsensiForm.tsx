...
export default function AbsensiForm({ event }: Props) {
  ...
  // Active form fields (excluding info blocks from response collection)
  const inputFields = (event.form_schema as FormField[]).filter((f) => f.type !== 'info');
    ...
    <form onSubmit={handleSubmit} className="glass-card rounded-2xl p-6 slide-up space-y-6">
      ...
      {nrpStatus === 'found' && anggota && (event.form_schema as FormField[]).length > 0 && (
        <div className="space-y-5 fade-in">
          ...
          {(event.form_schema as FormField[]).map((field, idx) => {
            // ── INFO BLOCK ──────────────────────────────────
            if (field.type === 'info') {
              return (
                <div key={idx} className="p-4 rounded-xl bg-amber-600/5 border border-amber-500/20">
                  {field.label && (
