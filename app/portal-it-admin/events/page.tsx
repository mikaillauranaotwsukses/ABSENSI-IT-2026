      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8 slide-up">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Kelola Formulir &amp; Kegiatan</h1>
              <p className="text-slate-400 text-sm">{events?.length ?? 0} formulir / kegiatan terdaftar</p>
            </div>
          </div>
          <Link
        ...
        <div className="tech-card rounded-2xl overflow-hidden slide-up border border-slate-700/60">
          {(!events || events.length === 0) ? (
            <div className="text-center py-16">
              ...
              <p className="text-slate-400">Belum ada formulir atau kegiatan. Buat yang pertama sekarang!</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-700/50">
                    <th className="text-left px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Nama Formulir / Kegiatan</th>
                    ...
                    <th className="text-center px-6 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Respon</th>
                    ...
                    return (
                      <tr key={ev.id} className="hover:bg-slate-800/30 transition-colors">
                          ...
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {evConfig.is_qr_enabled === false && (
                                <span className="px-1.5 py-0.5 rounded text-[10px] font-mono font-bold bg-slate-800 text-slate-400 border border-slate-700" title="Fitur Tiket QR dinonaktifkan (Form Murni)">
                                  Form Pendataan
                                </span>
                              )}
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className="inline-flex items-center justify-center min-w-[2rem] px-2.5 h-8 rounded-full bg-blue-600/20 text-blue-300 text-xs font-bold border border-blue-500/20">
                            {absensiCount}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-center">
